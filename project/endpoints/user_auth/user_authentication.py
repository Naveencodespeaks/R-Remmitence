...
async def register(request: Register, db: Session = Depends(get_database_session)):
    try:
        ...
        if len(str(mobile_no)) < 7 or len(str(mobile_no)) > 13:
        ...
        if user_obj.count() <=0:
            user_data = UserModel(role_id =2,status_id=1, email=email, mobile_no=mobile_no,password=AuthHandler().get_password_hash(Utility.uuid()))
            db.add(user_data)
            db.flush()
            db.commit()
            if user_data.id:
            ...
            if new_user.status_id == 1:
                return Utility.json_response(status=SUCCESS, message=all_messages.PENDING_EMAIL_VERIFICATION, error=[], data={},code="EMAIL_VERIFICATION_PENDING")
            elif  new_user.status_id == 2:
                #send Mai to user to complete profile
                Email()
                return Utility.json_response(status=FAIL, message=all_messages.PENDING_PROFILE_COMPLATION, error=[], data={},code="PENDING_PROFILE_COMPLATION")
                ...
                return Utility.json_response(status=SUCCESS, message=all_messages.PROFILE_INACTIVE, error=[], data={})
            elif new_user.status_id == 5:
                return Utility.json_response(status=FAIL, message=all_messages.PROFILE_DELETED, error=[], data={})
            else:
                db.rollback()
                return Utility.json_response(status=FAIL, message=all_messages.SOMTHING_WRONG, error=[], data={})
